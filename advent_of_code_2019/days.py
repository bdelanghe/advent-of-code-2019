"""all 25 days of advent of code 2019"""

from typing import List

from advent_of_code_2019.puzzle_input import get_input
...
class Day01:
    ...
    negative fuel should instead be treated as if it requires zero fuel; the remaining mass, if any,
    ...
    def __init__(self) -> None:
        self.module_masses: List[int] = [int(x) for x in get_input("day01_1")]
        self.module_fuel_requirements: List[int] = [
            self.mass_fuel_needed(x) for x in self.module_masses
        ]
        self.total_fuel_requirements: List[int] = [
            self.total_fuel_needed(x) for x in self.module_masses
        ]

    @property
    def module_total_fuel(self) -> int:
        """how much fuel total"""
        return sum(self.module_fuel_requirements)

    @property
    def grand_total_fuel(self) -> int:
        """how much fuel total including fuel for fuel"""
        return sum(self.total_fuel_requirements)

    @staticmethod
    def mass_fuel_needed(mass: int) -> int:
        """
    ...
    def total_fuel_needed(self, module_mass: int) -> int:
        """
        So, for each module mass, calculate its fuel and add it to the total. Then, treat the fuel amount you just
        calculated as the input mass and repeat the process, continuing until a fuel requirement is zero or negative.
        """
        total_fuel: int = 0
        additional_mass: int = module_mass
        while additional_mass != 0:
            additional_mass = self.mass_fuel_needed(additional_mass)
            total_fuel += additional_mass
        return total_fuel


if __name__ == "__main__":
    day = Day01()
    print(f'day 01 part 1: {day.module_total_fuel}')
    print(f'day 01 part 2: {day.grand_total_fuel}')
